...
import type { InfoDataTableServer, Leitor, Livro, Reserva } from '../../types';
...
import CancelarReservaForm from '../leitor/CancelarReservaForm.vue';
...
const constant: {
    ...
    cabecalhoReservas: [
        ...
        { title: "Leitor", align: "center", key: "r.leitor.cpf", sortable: true },
...
function loadItems({ search, page, itemsPerPage, sortBy }: InfoDataTableServer) {
    ...
    ReservaService.findSearchReservasByLivroId(page, itemsPerPage, sortBy, search, props.livroId, state.leitor?.id, state.selectedStatus)
...
<template>
        ...
        <v-data-table-server :search="state.search" :headers="constant.cabecalhoReservas" :items="state.listaReservas"
            ...
            <template v-slot:top>
                <div class="d-flex justify-start align-center mb-5">
                    ...
                    <v-btn @click="voltarParaLivros" color="error"
    ...
    <ReservaForm :dialog-visible="state.dialogVisible"
        :livro-id="props.livroId" @submitted="atualizarQuandoFormEnviado" @canceled="fecharModal" />
